...
- [ ] Task 1: Add JWT environment variables to config (AC: 4, 6)
  - [ ] Update `backend/src/core/config.py` to add: `JWT_SECRET: str`, `JWT_EXPIRES_IN: int = 3600`, `JWT_ALGORITHM: str = "HS256"`
  - [ ] Update `backend/.env.example` with JWT_SECRET placeholder and default values
  - [ ] Generate strong JWT_SECRET for local .env: `openssl rand -hex 32` (user action)

- [ ] Task 2: Implement JWT token creation (AC: 3, 4, 6)
  - [ ] Update `backend/src/core/security.py`
  - [ ] Import `jose.jwt` and `datetime`
  - [ ] Implement `create_access_token(user_id: str, expires_delta: Optional[timedelta] = None) -> str`
  - [ ] Function should: calculate expiry time, create payload with `{"sub": user_id, "exp": timestamp, "iat": timestamp}`, encode with JWT_SECRET and JWT_ALGORITHM
  - [ ] Default expires_delta to settings.JWT_EXPIRES_IN seconds

- [ ] Task 3: Create login request/response schemas (AC: 1, 3)
  - [ ] Update `backend/src/schemas/auth.py`
  - [ ] Define `LoginRequest` with email (EmailStr) and password (str)
  - [ ] Define `TokenResponse` with access_token (str), token_type (str = "bearer"), expires_in (int)

- [ ] Task 4: Implement login logic in auth service (AC: 2, 5, 8)
  - [ ] Update `backend/src/services/auth_service.py`
  - [ ] Implement `async def login(self, email: str, password: str) -> TokenResponse`
  - [ ] Find user by email using user_repository
  - [ ] If user not found, raise HTTPException(401, "Invalid credentials")
  - [ ] Verify password using verify_password(plain, hashed)
  - [ ] If password incorrect, raise HTTPException(401, "Invalid credentials")
  - [ ] Generate JWT token using create_access_token(user.id)
  - [ ] Return TokenResponse with access_token and expires_in

- [ ] Task 5: Create login route (AC: 1, 3, 5)
  - [ ] Update `backend/src/api/v1/auth.py`
  - [ ] Implement `POST /auth/login` endpoint
  - [ ] Endpoint accepts LoginRequest body
  - [ ] Call auth_service.login()
  - [ ] Return 200 with TokenResponse on success
  - [ ] 401 errors handled by service layer

- [ ] Task 6: Write unit and integration tests (AC: 7)
  - [ ] Update `backend/tests/test_auth.py`
  - [ ] Test successful login: valid credentials → 200 with JWT token
  - [ ] Test invalid email: non-existent email → 401
  - [ ] Test incorrect password: wrong password → 401
  - [ ] Test JWT structure: decode token, verify "sub", "exp", "iat" fields present
  - [ ] Test JWT expiry: verify exp timestamp is current_time + JWT_EXPIRES_IN
  - [ ] Test token signature: decode with JWT_SECRET succeeds
  - [ ] Test password verification: verify_password returns True for correct, False for incorrect
