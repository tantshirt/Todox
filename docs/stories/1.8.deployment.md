...
- [x] Task 1: Deploy backend to Railway (AC: 1, 2, 5)
  - [x] Created Railway project via CLI
  - [x] Set root directory to `backend` in Railway dashboard
  - [x] Railway automatically detects Python and FastAPI
  - [x] Configured environment variables via Railway CLI:
    - [x] `MONGODB_URI` - MongoDB Atlas connection string
    - [x] `JWT_SECRET` - Production secret (rotated for security)
    - [x] `JWT_EXPIRES_IN` - 3600
    - [x] `JWT_ALGORITHM` - HS256
    - [x] `CORS_ORIGINS` - Updated with Vercel URL
  - [x] Railway deployed successfully
  - [x] Backend URL: https://todox-backend-production.up.railway.app
  - [x] Health check verified: {"status":"healthy","database":"connected"}

- [x] Task 2: Configure backend for Railway deployment (AC: 1)
  - [x] Create `backend/Procfile` with: `web: uvicorn src.main:app --host 0.0.0.0 --port $PORT`
  - [x] Procfile provides explicit Railway start command
  - [x] Verify `requirements.txt` includes all dependencies
  - [x] `src/main.py` already properly configured

- [x] Task 3: Deploy frontend to Vercel (AC: 3, 4, 6)
  - [x] Deployed via Vercel CLI
  - [x] Vercel automatically detected Next.js
  - [x] Configured environment variable via CLI:
    - [x] `NEXT_PUBLIC_API_URL` - https://todox-backend-production.up.railway.app
  - [x] Vercel deployed successfully
  - [x] Frontend URL: https://frontend-dikdicjca-dres-projects-71e8c4e5.vercel.app
  - [x] Frontend accessible and building correctly

- [x] Task 4: Update CORS_ORIGINS on Railway (AC: 1)
  - [x] Updated CORS_ORIGINS via Railway CLI to include Vercel URL
  - [x] New value: `http://localhost:3000,https://frontend-dikdicjca-dres-projects-71e8c4e5.vercel.app`
  - [x] Railway redeployed automatically

- [x] Task 5: Create E2E test setup and basic smoke test (AC: 7)
  - [x] Create `e2e/` directory at project root
  - [x] Run `npm init -y` in e2e/ to create package.json
  - [x] Install Playwright: `npm install -D @playwright/test`
  - [x] Run `npx playwright install chromium` to install browser
  - [x] Create `e2e/playwright.config.ts` with:
    - [x] `baseURL: process.env.E2E_BASE_URL || 'http://localhost:3000'`
    - [x] Test directory: `./tests`
    - [x] Projects for chromium, firefox, webkit
  - [x] Create `e2e/tests/auth.spec.ts`
  - [x] Write smoke tests: complete auth flow, protected routes, validation
  - [x] Tests work against both local and production URLs
...
- [x] Task 7: Update README with deployment information (AC: 8)
  - [x] Update root `README.md`
  - [x] Add "Deployment" section with:
    - [x] Deployment instructions for Railway and Vercel (step-by-step)
    - [x] Environment variable requirements for each platform
    - [x] CORS configuration guide
    - [x] E2E testing commands
